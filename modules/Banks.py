...
class OtpBanka(Bank):
    def getRates(self):
        ...
        try:
            print(f"Midmarket rate is {indexes[0]}")
            print(f"OTP Banka cash buy rate EUR to HRK : {indexes[1]}")
            print(f"OTP Banka cash sell rate EUR to HRK : {indexes[2]}")
            print(f"OTP Banka foreign buy rate EUR to HRK : {indexes[3]}")
            print(f"OTP Banka foreign buy rate EUR to HRK : {indexes[4]}")
        except ValueError:
...
class ZABA(Bank):
    def getRates(self, type):
        ...
        try:
            ...
            indexes = [parsedText[eurIndex + 2], parsedText[eurIndex + 4]]
            print(f"ZABA {type} buy rate EUR to HRK : {indexes[0]}")
            print(f"ZABA {type} buy sell EUR to HRK : {indexes[1]}")
        except (Exception):
            print(f"Eror occurred while getting ZABA rates : {Exception}")
